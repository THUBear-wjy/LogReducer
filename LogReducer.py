...
def procFiles(typename, fileBeginNo, fileEndNo, now_input, now_output, now_temp, type_template):
    ...
    if (not os.path.exists(now_temp)):
        os.mkdir(now_temp)
    order = "./THULR -I " + now_input + " -X " + str(fileBeginNo) + " -Y " + str(fileEndNo) + " -O " + now_temp + " -T " + type_template + " -E " + encoder_mode + " -D " + time_diff + " -F " + os.path.join(type_template,"head.format")
